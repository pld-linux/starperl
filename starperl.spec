...
Pakiet StarPerl to zbiór modułów Perla dających dostęp do bibliotek z
infrastruktury Starlink. Aktualnie dostępne są interfejsy do systemu
komunikatów ADAM i systemu powiadamiania NBS. Dołączone są dodatkowe
moduły do określania wersji zainstalowanych pakietów Starlinka oraz do
...
Ten pakiet zawiera informacje o położeniu oprogramowania Starlink w
systemie. Ma za zadanie uprościć budowanie modułów Perla opartych na
bibliotekach Starlinka i pozwala na umieszczenie tych informacji w
jednym miejscu, zamiast w każdym module Starlinka osobno.
...
Ten moduł daje dostęp do rozszerzalnego formutu wielowymiarowych
danych (Starlink NDF) oraz związanych z nim funkcji obsługi błędów i
komunikatów. Załączone są także funkcje do dostępu do podstawowych
...
używa własnego formatu do zapisu linii widm i danych continuum UKT14.
Ten format jest nazywany GSD (Global Section Data - dane sekcji
globalnej). Ten moduł daje dostęp tylko do odczytu do danych JCMT.
...
Ten moduł udostępnia perlowy interfejs do wersji C lub fortranowej
